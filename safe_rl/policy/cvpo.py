...
class CVPO(Policy):
    ...
    def _update_actor(self, data):
        ...
        with torch.no_grad():
            ...
            # self.queue.append(target_qc_np)
            # if len(self.queue)==1:
            #     self.queue.append(target_qc_np)
            #     self.queue.append(target_qc_np)
        ...
        def dual(x):
            ...
            target_q_np_comb1 = target_q_np - lam * self.queue[0]
            target_q_np_comb2 = target_q_np - lam * self.queue[1]
            ...
            return η * self.dual_constraint + lam * self.qc_thres + np.mean(max_q) \
                ...
                # + η * np.mean(np.log(np.mean(np.exp((target_q_np_comb1 - max_q[:, None]) / η), axis=1)))\
                # + η * np.mean(np.log(np.mean(np.exp((target_q_np_comb2 - max_q[:, None]) / η), axis=1)))
